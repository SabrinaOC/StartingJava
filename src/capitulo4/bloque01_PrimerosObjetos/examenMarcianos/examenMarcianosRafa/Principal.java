...
public class Principal {
	 ...
	 * @return
	 */
	public static boolean quedanPersonajesVivosEnArray(Personaje array[]) {
		for (int i = 0; i < array.length; i++) { // Recorremos array
			if (array[i].isVivo()) { // Si encontramos uno vivo, devolvemos true
				return true;
			}
		}
		return false;
	...
	public static void diparos(CampoDeBatalla campo) {
		...
		do {
			...
			campo.cambiarEstadosAMuerto();

		} while ((quedanPersonajesVivosEnArray(campo.getArrayHumanos())) && quedanPersonajesVivosEnArray(campo.getArrayMalvados()));
	...
	public static void mostrarGanador (CampoDeBatalla campo) {
		// Mostramos ganador
				if (quedanPersonajesVivosEnArray(campo.getArrayHumanos())) {
					System.out.println("\t¡HAN GANADO LOS HUMANOS!");

				}
				if (quedanPersonajesVivosEnArray(campo.getArrayMalvados())) {
