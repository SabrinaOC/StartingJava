...
public class Ejercicio6_comprobar_matriz_dispersa {
	...
	public static boolean comprobarMatrizDispersa(int matriz[][]) {
		int contadori = 0; // Creamos contador para ver cu�ntos 0 encontramos
		int contadorj = 0; // Creamos contador para ver cu�ntos 0 encontramos
		...
		for (int j = 0; j < matriz.length; j++) { // Recorremos matriz al rev�s para comprobar nulos columnas
		...
		if (contadori == matriz.length && contadorj == matriz.length) { //Establecemos condici�n para devolver boolean
			esDispersa = true;
		}
		return esDispersa;
	}
